...
class ThirdPartyPunishment(environment.Environment):
    ...
    def __init__(self, num_inner_steps: int, num_outer_steps: int):
        ...
        def _step(
            ...
            ###################
            # # TODO - uncomment to hardcode punishment for defect
            # pl1_punished = jnp.where(
            #     prev_first_cd_actions[0] == 1, 1, 0
            # ) + jnp.where(prev_second_cd_actions[0] == 1, 1, 0)
            # pl2_punished = jnp.where(
            #     prev_first_cd_actions[1] == 1, 1, 0
            # ) + jnp.where(prev_second_cd_actions[1] == 1, 1, 0)
            # pl3_punished = jnp.where(
            #     prev_first_cd_actions[2] == 1, 1, 0
            ...
            pl1_intrinsic = (
                intrinsic * (pl1_rew_for_pl2_punish + pl1_rew_for_pl3_punish)
                + pl1_num_punishes * punish_cost
            )
            ...
            pl2_intrinsic = (
                intrinsic * (pl2_rew_for_pl1_punish + pl2_rew_for_pl3_punish)
                + pl2_num_punishes * punish_cost
            )
            ...
            pl3_intrinsic = (
                intrinsic * (pl3_rew_for_pl1_punish + pl3_rew_for_pl2_punish)
                + pl3_num_punishes * punish_cost
            )
