...
class ThirdPartyRandom(environment.Environment):
    ...
    def __init__(self, num_inner_steps: int, num_outer_steps: int):
        ...
        def _step(
            ...
            punisher_intrinsic_reward = jnp.where(
                punish_actions[old_punisher] % 2 == 1, 1, 0
            ) * jnp.where(prev_cd_actions[old_pl1] == 1, 1, 0,) + jnp.where(
                punish_actions[old_punisher] > 1, 1, 0
            ) * jnp.where(
                prev_cd_actions[old_pl2] == 1, 1, 0
            )

            intrinsic = (
                params.intrinsic
                * jnp.array([0, 0, punisher_intrinsic_reward])[
                    player_selection_lookup
                ]
            )

            punisher_cost = params.punish_cost * (
                jnp.where(punish_actions[old_punisher] % 1 == 1, 1, 0)
                + jnp.where(punish_actions[old_punisher] > 1, 1, 0)
            )

            cost = jnp.array([0, 0, punisher_cost])[player_selection_lookup]
            rewards = rewards + intrinsic + cost
