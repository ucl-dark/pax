...
def train(agent_0, agent_1, seed, num_exploration_episodes, train_steps, batch_size):
    ...
    NUM_ENVS = 2 ** int(log2(num_exploration_episodes))

    print("Iterated Prisoners Dilemma")
    print(f"Max time limit {FLAGS.max_episode_length}")
    print(f"Exploring for {num_exploration_episodes} episodes with {NUM_ENVS} envs")
    print(f"Training for {train_steps} episodes with batch size: {batch_size}")
    print("-----------------------")

    for i in range(train_steps):
        print(f"Exploration step {i}")
        for _ in range(num_exploration_episodes // (NUM_ENVS * train_steps) + 1):
            rewards = []
...
def main(_):
    ...
    agent_0, agent_1 = train(
        ...
        agent_1,
        FLAGS.seed,
        FLAGS.num_exploration_episodes,
        FLAGS.train_steps,
        FLAGS.batch_size,
    )
    ...
    rewards = [
        evaluate(agent_0, opponent, FLAGS.seed, FLAGS.max_episode_length, 1, True)[
            :, :, 0
        ]
