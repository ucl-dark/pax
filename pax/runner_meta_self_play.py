...
class EvoMetaSelfPlayRunner:
    ...
    def train_loop(self, env, agents, num_episodes, watchers):
        ...
        lamb = 1.0
        lamb_anneal = 0.0005  # Anneals to 0 after 10
        for gen in range(num_iters):
            ...
            # Player 2
            # TODO: Add meta-self-play:
            # Anneal away lambda after each generation
            #########################################
            if lamb > 0:
                lamb -= lamb_anneal
            print(f"Generation: {gen}")
            print(f"Lambda: {lamb}")
            if jax.random.uniform(rng_lamb, minval=0.0, maxval=1.0) > lamb:
                print("vs. itself")
                a2_state, a2_mem = a1_state, a1_mem
            else:
                print("vs. NL")
