...
class SampleSynq(NamedTuple):
    ...
    behavior_log_probs: jnp.ndarray
    behavior_values: jnp.ndarray
    behavior_synq: jnp.ndarray
    target_synq: jnp.ndarray
    dones: jnp.ndarray
    hiddens: jnp.ndarray
...
class ActRunner:
    ...
    def __init__(
        ...
        agent2.batch_synq_value = jax.jit(jax.vmap(jax.vmap(agent2._synq, 0, 0)))
        ...
        def _inner_rollout_train(carry, unused):
            ...
            synq2, a2_state, new_a2_mem = agent2.batch_policy(
                a2_state,
                obs2,
                new_a2_mem,
            )
        ...
        def _outer_rollout_train(carry, unused):
            """Runner for trial"""
            # play episode of the game
            vals, trajectories = jax.lax.scan(
                _inner_rollout_train,
                carry,
                None,
        ...
        def _inner_rollout_test(carry, unused):
            """Runner for inner episode"""
            (
                rngs,
                obs1,
            ...
            synq2, a2_state, new_a2_mem = agent2.batch_synq_value(
                a2_state,
                obs2,
                a2_mem,
            )

            obs2 = jnp.concatenate([obs1, synq2], axis=-1)
        ...
        def _outer_rollout_test(carry, unused):
            """Runner for trial"""
            # play episode of the game
            vals, trajectories = jax.lax.scan(
                _inner_rollout_test,
        ...
        def _rollout(
            ...
            obs1, env_state = env.reset(rngs, _env_params)
            rewards = [
            ...
            # Player 2
            if args.agent2 == "NaiveEx":
                a2_state, a2_mem = agent2.batch_init(obs1)
            ...
            # Test Episodes
            # Reset Environment and Returns for Test Episodes
            obs1, env_state = env.reset(rngs, _env_params)
            rewards = [
                jnp.zeros((args.popsize, args.num_opps, args.num_envs)),
                jnp.zeros((args.popsize, args.num_opps, args.num_envs)),
            ...
            # Reset Shaper's memory
            _a1_mem = agent1.batch_reset(_a1_mem, False)

            # Reinitialize Player 2 for Test Episodes
            # Player 2
            if args.agent2 == "NaiveEx":
                a2_state, a2_mem = agent2.batch_init(obs1)
