...
class DiscreteDoubleCritic(nn.Module):
    action_dim: int

    @nn.compact
    def __call__(self, state, Q1=False):
        # q1 = nn.Dense(features=64)(state)
        # q1 = nn.LayerNorm()(q1)
        # q1 = nn.tanh(q1)
        ...
        q1 = nn.Dense(features=self.action_dim)(state)

        if Q1:
            return q1

        # q2 = nn.Dense(features=64)(state)
        # q2 = nn.LayerNorm()(q2)
        # q2 = nn.tanh(q2)
        ...
        q2 = nn.Dense(features=self.action_dim)(state)
        return q1, q2
...
class DoubleCritic(nn.Module):
    ...
    def __call__(self, state, action, Q1=False):
        ...
        q1 = nn.Dense(features=1)(state_action)
        ...
        q2 = nn.Dense(features=1)(state_action)
        return q1, q2
...
def build_categorical_policy_model(input_shapes, action_dim, init_rng):
    init_batch = jnp.ones(input_shapes, jnp.float32)
    policy = CategoricalPolicy(action_dim=action_dim)
    init_variables = policy.init(init_rng, init_batch, init_rng, False)

    return init_variables["params"]


@partial(jax.jit, static_argnums=(1, 4))
def apply_categorical_policy_model(
    params: FrozenDict,
    action_dim: int,
    state: jnp.ndarray,
    key: PRNGSequence,
    ...
    return CategoricalPolicy(action_dim=action_dim).apply(
        dict(params=params), state, key, eval
    )


def build_discrete_double_critic_model(input_shapes, action_dim, init_rng):
    init_batch = jnp.ones(input_shapes, jnp.float32)
    critic = DiscreteDoubleCritic(action_dim=action_dim)
    init_variables = critic.init(init_rng, init_batch)
    return init_variables["params"]


@partial(jax.jit, static_argnums=(1, 3))
def apply_discrete_double_critic_model(
    params: FrozenDict, action_dim: int, state: jnp.ndarray, Q1: bool
) -> jnp.ndarray:
    return DiscreteDoubleCritic(action_dim=action_dim).apply(
        dict(params=params), state, Q1=Q1
    )
