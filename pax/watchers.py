...
def policy_logger(agent) -> None:
    weights = agent.actor_optimizer.target["Dense_0"]["kernel"] #[layer_name]['w']
    log_pi = nn.softmax(weights)
    probs = {"policy/" + str(s): p[0] for (s, p) in zip(State, log_pi)} # probability of cooperating is p[0] 
    return probs
...
def policy_logger_dqn(agent) -> None:
    ...
    probs.update({f"policy/{str(s)}.defect": p[1] for (s, p) in zip(State, pi)})
...
def value_logger_dqn(agent) -> None:
    ...
    values.update(
        {"value/target_update_steps": agent.num_target_updates}
    )
    return values
